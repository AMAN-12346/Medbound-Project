...
const { createTestimonial, findTestimonial, updateTestimonial } =
  TestimonialServices;
...
export class staticController {
  ...
  async editTestimonial(req, res, next) {
    const validationSchema = {
      _id: Joi.string().required(),
      Testimonial_no: Joi.string().optional(),
      title: Joi.string().optional(),
      description: Joi.string().optional(),
    };
    try {
      const validatedBody = await Joi.validate(req.body, validationSchema);
      let staticRes = await findTestimonial(
        { _id: req.body._id },
        { status: { $ne: status.DELETE } }
      );
      if (staticRes) throw apiError.notFound(responseMessage.TESTIMONIAL_NOT_FOUND);
